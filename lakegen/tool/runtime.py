...
class ToolRuntime:
    def invoke(self, kind: str, tools_to_call: dict[str, Any]) -> list[ToolOutput]:
        return [
            self.use_tool(kind, name, params)
            for name, params in tools_to_call.items()
        ]

    def use_tool(self, kind: str, name: str, params: dict[str, Any]) -> ToolOutput:
        try:
            ...
            schema = registry.get_tool_schema(kind, name)
            ...
            handler = registry.get_tool_handler(kind, name)
